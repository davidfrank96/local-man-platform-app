...
import { cleanupQaAdminVendors } from "../../../../../lib/admin/vendor-service.ts";
...
export async function DELETE(request: Request) {
  ...
  try {
    const result = await cleanupQaAdminVendors({ session: admin.session });

    return apiSuccess(result);
  } catch (error) {
    return handleAdminServiceError(error, "Unable to cleanup QA Admin vendors.");
  }
}
